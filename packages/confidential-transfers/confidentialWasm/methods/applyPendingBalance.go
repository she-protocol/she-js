...
// Creates a new ApplyPendingBalnaceProto struct that can be sent to the chain.
...
func NewApplyPendingBalanceWASM(this js.Value, args []js.Value) interface{} {
	...
	// 4. Parse CurrentDecryptableBalance (Base64-encoded string -> []byte)
	currentDecryptableBalance := args[3].String()
	...
	// 5. Parse Amount (string -> *big.Int)
	currentPendingBalanceCounter := args[4].Int()
	...
	if err != nil {
		fmt.Println("WD Error:", err)
		return js.ValueOf(fmt.Sprintf("error: %s", err))
	}

	// 7. Parse PendingBalanceLo (Base64-encoded string -> []byte)
	pendingBalanceLoBase64 := args[6].String()
	...
	// 8. Parse PendingBalanceHi (Base64-encoded string -> []byte)
	pendingBalanceHiBase64 := args[7].String()
	...
	pendingBalanceHi, err := pendingBalanceHiProto.FromProto()
	if err != nil {
		return js.ValueOf(fmt.Sprintf("error: %s", err))
	}

	// Create the applyPendingBalance sturct
	applyPendingBalance, err := types.NewApplyPendingBalance(signedDenom, address, denom, currentDecryptableBalance, uint16(currentPendingBalanceCounter), currentAvailableBalance, pendingBalanceLo, pendingBalanceHi)
	if err != nil {
		fmt.Println("Apply Pending Balance Creation Error:", err)
		return js.ValueOf(fmt.Sprintf("error: %s", err))
	}
	...
	returnString, err := json.MarshalIndent(applyPendingBalanceProto, "", "  ")
	if err != nil {
